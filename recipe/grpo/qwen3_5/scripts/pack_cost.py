...
def step(seqs, cu):
    ...
    kw = {"position_ids": pos}
    if cu is not None:
        kw["cu_seqlens"] = torch.tensor([0, *torch.tensor(seqs).cumsum(0).tolist()], device="cuda", dtype=torch.long)
...
def timeit(label, fn, tokens):
    ...
    print(f"  {label:44s} {dt:7.3f} s   {dt / tokens * 1e6:7.1f} us/tok   peak {torch.cuda.max_memory_allocated() / 2**30:5.1f} GB")
    return dt
...
print("  A vs 2x(one seq T): quadratic check")
